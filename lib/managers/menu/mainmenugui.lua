...
function MainMenuGui:init(ws, fullscreen_ws, node)
	...
	self._enabled = true
	self._update_boxes = {}
	self._data = node:parameters().menu_component_data or {selected_box = "play", current_state = "default"}
...
function MainMenuGui:_update_box_status(box, selected)
	...
	if text_object and text_object.selected_text and text_object.unselected_text then
		...
		local shrink_text = not box.params or box.params.shrink_text or true
		local adept_width = box.params and box.params.adept_width or false
		local keep_box_ratio = not box.params or box.params.keep_box_ratio or true
